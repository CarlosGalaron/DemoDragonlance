...
function ScenarioDelta() {
	...
	const [gameState, setGameState] = useState(state.gameNotStartedState); // Estado del juego
	...
	switch (gameState) {
		case 'INIT': {
			...
			content =
				<div className='Scenario-init-father'>
					<h3 className='Scenario-init-title'>EL REENCUENTRO FRUSTRADO</h3>
					<div className='Scenario-init-intro'>
						<p>De camino a solace los hermanos Raistlin y Caramón se encontraron con Sturm.</p>
						<p>Hacía cinco largos años que no se veían, pero como prometieron en la posada de "El último hogar", los tres habían vuelto a su hogar de la infancia para contarse el resultado de la búsqueda en la que se habían embarcado.</p>
						<p>Apenas si habían intercambiado un breve saludo cuando les salieron al paso tres oscuras figuras desde el lindero del bosque.</p>
						<p>Los compañeros se vieron obligados a postergar su reecuentro y a empuñar las armas con premura...</p>
					</div>
					<button className='Scenario-init-button' onClick={() => setGameState('COMBAT')}>Comenzar</button>
				</div>
		...
		case 'VICTORY': {
			content =
				<div className='Scenario-endgame-father'>
					<div className='Scenario-endgame-title'>
						<h1>¡VICTORIA!</h1>
					</div>
					<div className='Scenario-endgame-intro'>
						<p>Tras un duro combate, los héroes derrotaron a sus contrincantes.</p>
						<p>Curaron sus heridas y se apresuraron a llegar a Solace, estaba claro que los caminos ya no eran seguros</p>
						<p>Desgraciadamente para ellos, esto no sería más que el preludio de muchos desafiós...</p>
					</div>
					<div className='Scenario-endgame-button' onClick={() => {
						setGameState('INIT')
						resetGame()
					}}>
						Reiniciar
		...
		case 'DEFEAT': {
			content =
				<div className='Scenario-endgame-father'>
					<div className='Scenario-endgame-title'>
						<h1>Fran te ha suspendido</h1>
					</div>
					<div className='Scenario-endgame-intro'>
						<p>Los tres amigos fueron derrotados, horas después despertaron malheridos y encadenados, sin saber que sería de ellos.</p>
					</div>
					<div className='Scenario-endgame-button' onClick={() => {
						setGameState('INIT')
						resetGame()
